...
const ExpandedNewTeam = ({ fragment, globalPageProps }: Props) => {
  const {
    title: header,
    subtitle: subheader,
    showcasedNewTeam: newteam,
  ...
  return (
    <Box id="ExpandedNewTeam" ref={ref}>
      <Container maxW="container.xl" py={10} px={8}>
        ...
        >
          <Slider {...settings}>
            {newteam.map((cast, index) => {
              const middleIndex = (currentSlide + 1) % newteam.length;

              const isActive = index === middleIndex;

              const isPrev =
                index === currentSlide - 1 ||
                (currentSlide === 0 && index === newteam.length - 1);
              const isNext =
                index === currentSlide + 1 ||
                (currentSlide === newteam.length - 1 && index === 0);
              ...
              return (
                ...
                >
                  <ChakraLink
                    role="group"
                    href={buildUrl(globalPageProps, `/posts/team/${cast.slug}`)}
                    borderRadius="xl"
                    bg="#2a2a2a"
                    boxShadow="lg"
                    p={6}
...
export default ExpandedNewTeam;
